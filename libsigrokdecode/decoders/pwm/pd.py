...
class Decoder(srd.Decoder):
    ...
    def decode(self):
        ...
        ssnible_block = self.first_samplenum
        ...
        while True:
            ...
            # @kripthor - Report bits and nibbles
            if (percent > 50): 
                bstring = '1'
            else:
                bstring = '0'
            self.put(self.ss_block, self.es_block, self.out_ann, [2, [bstring]])
            nstring += bstring
            if (len(nstring) == 4):
                nstring = hex(int(nstring, 2))[2:]
                self.put(ssnible_block, self.es_block, self.out_ann, [3, [nstring]])
                nstring = ''
                ssnible_block = self.es_block
